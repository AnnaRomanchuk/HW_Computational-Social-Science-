# -*- coding: utf-8 -*-
"""Copy_Romanchuk "HW2_3_python_basics.ipynb"

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1J_KtSEW-qHyQxFtnmhpM5-JJK0M7uoa3
...
def copy(seq):
    return type(seq)([x for x in seq])
...
def digits_to_words(text):
  ...
  return " ".join([mapping[ch] for ch in text if ch.isdigit()])
...
# EXAMPLE
print(format_date('2020-02-06 23:25:41')) # -> 26350525.683333334
...
print(df[['SubmitTime', 'AcceptTime']].head())
...
def get_work_time(df):
    per_hit = df['SubmitTime'] - df['AcceptTime']
    return per_hit.groupby(df['WorkerId']).sum()
...
def calculated_work_time(df):
    def sum_non_overlapping(g):
        ivals = g[['AcceptTime', 'SubmitTime']].sort_values('AcceptTime').to_numpy()
        if len(ivals) == 0:
            return 0.0
        total = 0.0
        cur_start, cur_end = ivals[0]
        for a, b in ivals[1:]:
            if a <= cur_end:
                if b > cur_end:
                    cur_end = b
    ...
    return df.groupby('WorkerId', sort=False).apply(sum_non_overlapping).astype(float)

# EXAMPLE
# Series should look the same as above but with different values for some workers
rwt = calculated_work_time(df)
print(rwt.head())
...
def get_film_data(df_list):
    target = {"Rank", "Title", "Worldwide gross"}
    for df in df_list:
        if isinstance(df.columns, pd.MultiIndex):
            cols = [" ".join([str(x) for x in tup if pd.notna(x)]).replace("\xa0"," ").strip()
                    for tup in df.columns.to_list()]
        else:
            cols = [str(c).replace("\xa0"," ").strip() for c in df.columns]

        if target.issubset(set(cols)):
            out = df.copy()
            out.columns = cols
            return out

top_films = get_film_data(films_2010)
print(top_films.columns.tolist())
print(top_films.head(3))
...
def extract_revenue(df, column, name):
    def to_millions(val):
        try:
            amt = price_dec(str(val))
            if amt is None:
                return pd.NA
            return round(float(amt) / 1_000_000, 2)
        except Exception:
            return pd.NA

    out = df.copy()
    out[name] = out[column].apply(to_millions)
    return out

top_films_revenue = extract_revenue(top_films_ranked, 'Worldwide gross', 'Revenue (millions')
print(top_films_revenue[['Worldwide gross', 'Revenue (millions']].head())
